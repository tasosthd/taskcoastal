  ...
  try {
    if (!process.env.RESEND_API_KEY) {
      return {
        ...
        body: JSON.stringify({
          error: "Missing RESEND_API_KEY in Netlify environment variables."
        })
      };
    }
        ...
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:580px; background:linear-gradient(145deg,#123f4f,#092b37); border-radius:30px; overflow:hidden; border:1px solid rgba(255,255,255,0.14); box-shadow:0 28px 80px rgba(0,0,0,0.35);">
          ...
          <tr>
            <td align="center" style="padding:30px 32px 12px 32px;">
              <a href="${cleanSiteUrl}" style="display:inline-block; padding:16px 26px; background:linear-gradient(135deg,#65d9f5,#22a9c9); color:#061a22; text-decoration:none; border-radius:18px; font-size:15px; font-weight:900;">
                Visit Coastal Flow
              </a>
            </td>
          </tr>
...
`;
    ...
    console.error("send-welcome.js error:", error);
